...
--given a NxN matrix return the product of adjacents numbers in the same direction 
maxOfMatrix :: Ord a => Num a => Matrix a -> a
maxOfMatrix m = maximum $ [(product.getDiagonal) m ] 
    ++ (map product $ map (getRow     m) [0,1..length m -1]) -- the product of each row
    ++ (map product $ map (getColumn  m) [0,1..length m -1])
...
--assuming NxN matrix and valid index 0,..,N-1
getDiagonal :: Matrix a -> [a]
getDiagonal m = helper m 0 where 
    helper m n 
        |length m == n = []
        |otherwise = ((m!!n)!!n): helper m (n+1) 

getRow :: Matrix a -> Int-> [a]
getRow m n = helper m n 0 where 
    helper m n z
        |length m == z = []
        |otherwise = ((m!!n)!!z): helper m n (z+1) 

getColumn :: Matrix a -> Int -> [a]
getColumn m n = helper m n 0 where 
    helper m n z
        |length m == z = []
